using Sitecore.Resources.Media;

namespace EMAAR.ECM.Foundation.Customcontrols.Fields.Extensions
{
    public static class AdvancedFieldExtensions
    {
        public static string GetUrl(this AdvanceImageField imageField, int width = 0, int height = 0)
        {
            if (imageField == null)
            {
                return string.Empty;
            }

            if (width <= 0 || height <= 0)
            {
                return imageField.Src.Replace("/sitecore/shell", "");
            }

            var src = string.Format("{0}?cx={1}&cy={2}&cw={3}&ch={4}",
                imageField.Src, imageField.CropX, imageField.CropY, width, height);

            var hash = HashingUtils.GetAssetUrlHash(src.Replace("/sitecore/shell", ""));

            return string.Format("{0}&hash={1}", src, hash);
        }
    }
}